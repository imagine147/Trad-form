...
const Business = () => {
    ...
    const onSubmit = (data) => {
      ...
      console.log(data)
      localStorage.setItems('formData', JSON.stringify(data))
      ...
      <div className='grid  justify-items-center w-[80%] mx-auto'>
        <form  action="" onSubmit={handleSubmit (onSubmit)}>
          ...
          <div className=' grid grid-cols gap-6 py-8 text-sm'>
            ...
            <input className='bg-white py-2 rounded-sm px-6' type="text" placeholder='Business Name' 
            {...register('businessName',{required: 'This field is required',
              pattern: {
                value: /^[a-zA-Z]+(([',. -][a-zA-Z ])?[a-zA-Z]*)*$/,
                message: "This field is required"
              }})} />
            {errors.businessName && <p style={{color: "#d73c3c"}}>{errors.businessName.message}</p>}
          ...
          <button className='bg-[#7C20BF] rounded-sm w-full py-2 text-white ' type='submit'>
            Sign Up
          </button>
          </div>
